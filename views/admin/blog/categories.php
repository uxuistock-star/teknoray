...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 text-sm">Blog kategori listesini yönetin</p>
        </div>
    </div>

    <div class="bg-white rounded-xl border border-gray-100 p-6">
        <form action="/yonetim/blog/kategoriler/kaydet" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                ...
                <?php foreach (($categories ?? []) as $cat): ?>
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-right">
                            <form action="/yonetim/blog/kategoriler/sil/<?= (int) $cat['id'] ?>" method="POST" onsubmit="return confirm('Bu kategoriyi silmek istediğinizden emin misiniz?')" class="inline">
                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($csrf_token ?? '')) ?>">
